...
const EditarPatrimonioModal = ({ open, onOpenChange, item }: EditarPatrimonioModalProps) => {
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        ...
        valor: item.valorAquisicao.toString(),
        status: item.status,
  ...
  const handleFotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const existingCount = (item?.fotos?.length || 0) + fotos.length;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('Editando patrimônio:', { 
      id: item?.id, 
      formData, 
      novasFotos: fotos, 
    ...
    toast({
      ...
      description: "Dados do patrimônio atualizados com sucesso",
    });
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="localFisico">Local *</Label>
              <Input
                id="localFisico"
            ...
            <div className="space-y-2">
              <Label htmlFor="valor">Valor do Bem (R$)</Label>
              <Input
                id="valor"
                type="number"
                step="0.01"
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm text-muted-foreground">
                {(item.fotos?.length || 0) + fotos.length}/5 fotos
              </span>
            </div>
